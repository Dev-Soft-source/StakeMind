...
async def test_premium_advanced_scores_requires_premium(app, mock_db_session) -> None:
    with patch("app.api.v1.dependencies.is_wallet_premium", new_callable=AsyncMock, return_value=False):
...
async def test_premium_advanced_scores_ok_when_entitled(app, mock_db_session) -> None:
    ...
    with patch("app.api.v1.dependencies.is_wallet_premium", new_callable=AsyncMock, return_value=True):
        with patch(
...
async def test_redeem_invite_success(app, mock_db_session) -> None:
    ...
    with patch("app.api.v1.entitlements.redeem_invite_code", fake_redeem):
        with patch("app.api.v1.entitlements.audit_service.record_audit_event", new_callable=AsyncMock):
            app.dependency_overrides[get_db_session] = mock_db_session
            try:
                async with app.router.lifespan_context(app):
                    transport = ASGITransport(app=app)
